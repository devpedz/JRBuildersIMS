...
<div class="page-body">
                        ...
                        <div class="header-top">
                            ...
                            <div>
                                <button class="btn btn-primary" type="button" data-bs-toggle="modal"
                                    data-bs-target="#modal_add">New Cash Advance </button>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                            ...
                            <div class="col-sm-6 col-md-3">
                                <div class="mb-3 form-floating">
                                    <input class="form-control" type="text" id="search" name="search" placeholder=""
                                        required="" oninput="loadCashAdvance()">
    ...
    <div class="modal fade" id="modal_add" tabindex="-1" role="dialog" aria-labelledby="modal_add" aria-hidden="true">
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                <div class="modal-toggle-wrapper social-profile text-start dark-sign-up">
                    <h4 class="modal-header justify-content-center border-0 text-dark">New Cash Advance</h4>
                    <div class="modal-body">
                        <form id="form_add" class="row g-3 needs-validation" novalidate="">
                            ...
                            <input type="text" name="rate_per_day" id="rate_per_day" required hidden>
                            <div class="col-md-6 col-sm-12">
                                <div class="mb-3 form-floating">
                            ...
                            <div class="col-md-6 col-sm-12">
                                <div class="mb-3 form-floating">
                                    <input id="amount" name="amount" class="form-control cash" type="number" required
                                        placeholder>
                            ...
                            <div class="col-md-6 col-sm-12">
                                <div class="mb-3 form-floating">
                                    <input id="weeks_to_pay" name="weeks_to_pay" class="form-control cash" type="number"
                                        required placeholder>
                                    <label class="form-label">Weeks to pay</label>
                                </div>
                            </div>
                            <div class="col-md-6 col-sm-12">
                                <div class="mb-3 form-floating">
                                    <input id="weekly_deduction" name="weekly_deduction" class="form-control cash"
                                        type="text" readonly required placeholder>
                                    <label class="form-label">Weekly Deduction</label>
                                </div>
                            </div>
                            <div class="col-md-12">
                                <div class="form-group">
...
<div class="modal fade" id="deleteCashAdvance" tabindex="-1" role="dialog" aria-labelledby="deleteCashAdvance"
    aria-hidden="true">
        ...
        <div class="modal-content">
            <div class="modal-body">
                <div class="modal-toggle-wrapper">
                    ...
                    <p class="text-center">Are you sure you want to delete this record? <br>This action cannot be
                        undone.</p>
                </div>
            </div>
...
<script>
    ...
    $('#employee').selectize({
        onChange: function() {
            $.post("/getRate", {
                ...
                function(rate) {
                    $('#amount').attr('min', rate);
                    $('#amount').attr('max', (rate * 5))
    ...
    $('.delete').click(function(e) {
        $.post("/deleteCashAdvance", {
                id: attendanceId
            },
            function(data, textStatus, jqXHR) {
                if (data.success) {
                    loadCashAdvance();
                    $('#deleteCashAdvance').modal('hide');
                }
            },
            "json"
        );
    ...
    // Example usage: Check the input value against the max
    amountInput.addEventListener('input', function() {
        ...
        if (amount > maxAmount) {
            Swal.fire({
                ...
                text: `The amount cannot exceed ${formatNumber(maxAmount)}!`,
            });
            this.value = ''; // Optionally clear the input field
        }
    });
    $('#form_add').submit(function(e) {
        if ($('#weekly_deduction').val() === '') {
            return false;
        }
        if (this.checkValidity()) {
            $.post("/addCashAdvance", $(this).serialize(),
                function(data) {
                    if (data.success) {
    ...
    function updateCashAdvance(id) {
        ...
        $('#form_update').submit(function(e) {
            if (this.checkValidity()) {
                $.post("/updateCashAdvance", $(this).serialize(),
                    function(data) {
                        if (data.success) {
    ...
    function updateDeduction() {
        const deduction = $('#weeks_to_pay').val() == 0 ? null : $('#amount').val() / $('#weeks_to_pay').val();
        $('#weekly_deduction').val(deduction);
    }
    $(document).on('input', '#amount', function() {
        updateDeduction();
    });
    $(document).on('input', '#weeks_to_pay', function() {
        updateDeduction();
    });
    $(function() {
