...
function storeSample(sample) {
    ...
    //{"id":0,"index_finger":[],"middle_finger":[]}
    let samples = JSON.parse(sample.samples);
    let sampleData = samples[0].Data;
    console.log(sampleData);
    // console.log(data);
    var id = "123";
    // console.log(finger_verify['index_finger'].length);
    if (verification_count >= 6) {
        var finger_scan = [sampleData];
        var finger_verify = myReader.currentHand.generateFullHand();
        var data = (JSON.stringify({ id, finger_scan, finger_verify }));
        let payload = `data=${data}`;
        api.post('/src/core/verification.php', payload)
            .then(data => {
                console.log(data);
    ...
    // .then(data => {
    //     if (data.status == 'success') {
    //         clockInAlert(data.fullname, data.imageUrl);
